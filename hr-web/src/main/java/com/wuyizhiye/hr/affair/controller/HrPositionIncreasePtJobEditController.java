 ...
 */
@Controller
@RequestMapping(value="hr/positionincreaseptjob/*")
public class HrPositionIncreasePtJobEditController extends EditController{
	...
	protected Object getSubmitEntity(){
		...
		if(StringUtils.isEmpty(bill.getBillNumber())){
			...
			try {
				billNumber = GenerateKey.getKeyCode(null, ProcessTypeEnum.INCREASE_PARTTIMEJOB.toString());
			} catch (Exception e) {
		...
		bill.setTitle(bill.getApplyPersonName()+"申请 兼职于 "+bill.getApplyChangeOrg().getName()+"-"+
	...
	protected boolean validate(Object data) {
		...
		if("SUBMIT".equals(bill.getBillStatus())){
			...
			params.put("orgId", bill.getApplyChangeOrg().getId());
			params.put("position", bill.getApplyChangePosition().getId());
			params.put("jobLevelId", bill.getApplyChangeJoblevel().getId());
			List<PersonPosition> ppList = personPositionDao.getPerPositionListByCondition(params);
			if(ppList!=null && ppList.size()>0){
				getOutputMsg().put("MSG", "员工["+bill.getApplyPersonName()+"]已有["+bill.getApplyChangePosition().getName()+"]的兼职,不能提交！");
				return false;
			}
		...
		if(validate(data)){
			if(data instanceof CoreEntity){
				...
				this.processViewService.generateNWFProcessView(phb
						.getTitle(), phb.getId(), ProcessTypeEnum.INCREASE_PARTTIMEJOB,
