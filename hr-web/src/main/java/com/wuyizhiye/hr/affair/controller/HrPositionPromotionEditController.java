...
public class HrPositionPromotionEditController extends EditController{
	...
	protected Object getSubmitEntity(){
		...
		if(StringUtils.isEmpty(bill.getBillNumber())){
			...
			try {
				billNumber = GenerateKey.getKeyCode(null, ProcessTypeEnum.PROMOTION.toString());
			} catch (Exception e) {
		...
		bill.setTitle("申请将 "+bill.getApplyPersonName()+" 晋升为  "+bill.getApplyChangeOrg().getName()+"-"+
	...
	public void save(HttpServletResponse response) throws InstantiationException, IllegalAccessException{
		...
		if(validate(data)){
			if(data instanceof CoreEntity){
				...
				PositionHistoryBill bill = ((PositionHistoryBill)data);
				if(workFlowService.existsProcessDefinition("promotionProcess")){
					if(BillStatusEnum.SUBMIT.toString().equals(bill.getBillStatus()) &&StringUtils.isEmpty(bill.getProcessInstance())){
						startWorkFlow(bill);
					}else if(BillStatusEnum.SAVE.equals(bill.getBillStatus())){
						this.processViewService.generateWFProcessView(bill
								.getTitle(), bill.getId(),
								bill.getProcessInstance(), ProcessTypeEnum.PROMOTION,
					...
					this.processViewService.generateNWFProcessView(bill
							.getTitle(), bill.getId(),ProcessTypeEnum.PROMOTION,
							bill.getBillStatus(), "HR",
							null == bill.getApplyPerson() ? ""
									: bill.getApplyPerson().getId(),
									null == bill.getApplyPerson() ? ""
	...
	private void startWorkFlow(PositionHistoryBill personPositionChange){
		if(workFlowService.existsProcessDefinition("promotionProcess")){
			...
			param.put("billPerson", personPositionChange.getApplyPerson().getId());//单据人员ID
			...
			variables.setViewPath("hr/positionpromotion/edit?VIEWSTATE=VIEW&id=${id}");
			ProcessInstance process = workFlowService.startWorkFlow("promotionProcess", personPositionChange.getId(), variables);
			...
			this.processViewService.generateWFProcessView(personPositionChange
					...
					personPositionChange.getProcessInstance(), ProcessTypeEnum.PROMOTION,
